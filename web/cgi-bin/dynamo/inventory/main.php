...
if (isset($_REQUEST['getData']) && $_REQUEST['getData']) {
  ...
  if ($data_type == 'size') {
    ...
    $sort_array = array(); // array to sort
    $cont_array = array(); // array with contents

    foreach ($repl_counts as $key => $array) {
      if (count($array) == 0) {
        $sort_array[$key] = 0.;
        $cont_array[$key] = array(0., 0.);
        continue;
      }
      ...
      $sort_array[$key] = $mean;
      $cont_array[$key] = array($mean, $rms);
    }

    arsort($sort_array);

    $content = &$data['content'];
    foreach ($sort_array as $key => $mean)
      $content[] = array('key' => $key, 'value' => $cont_array[$key]);
  }
