...
function initPage(dataType, categories, constraints) {
  var ajaxInput = {
    'url': 'http://' + location.hostname + ':8080/data/inventory/groups',
...
function setGroups(data) {
  d3.select('#group')
    ...
    .enter()
    .append('option')
    .attr('value', function (d) { if (d.name == '(no group)') return 'None'; else return d.name; })
...
function loadData() {
  ...
  var ajaxInput = {
    'url': 'http://' + location.hostname + ':8080/data/inventory/stats/' + dataType,
...
function getData() {
  ...
  var url = 'http://' + location.hostname + ':8080/data/inventory/stats/' + dataType
