            ...
            for loadDirectory in subdirs:
                ...
                for ifile in files:
                    ...
                    for entry in lines:
                        ...
                        bl_name = Block.convert2md5(bl_name)
            ...
            for bl_name in dsetObj.allKids():
                ...
                for rfile in blockObj.allKids():
                    ...
                    for csite in locations:
                        ...
                        #if not fileObj.locatedAtSite(csite):
                        #    continue
                        #dfile_replica = FileReplica(dfile,sites[csite],True,
                        #                            fileObj.sizeAtSite(csite))
                        #if bl_name in dataset_replica.block2file_replicas:
                        #    dataset_replica.block2file_replicas[bl_name].append(dfile_replica)
                        #else:
                        #    dataset_replica.block2file_replicas[bl_name] = [dfile_replica]
                        #sites[csite].add_file_replica(dfile_replica)
