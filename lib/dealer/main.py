...
class Dealer(object):
    ...
    def __init__(self, config):
        ...
        self.max_site_pending_volume = config.site_pending_volume * 1.e+12
    ...
    def run(self, inventory, comment = ''):
        ...
        copy_list = self._determine_copies(target_sites, partition, requests)
    ...
    def _determine_copies(self, target_sites, partition, requests):
        """
        ...
        copy_list = {}
        copy_volumes = dict((site, 0.) for target_sites) # keep track of how much we are assigning to each site
        ...
        for item, destination, plugin in requests:
            ...
            new_replica = make_new_replica(item, destination, self.policy.group)
            ...
            if sum(copy_volumes.itervalues()) > self.max_total_cycle_volume:
    ...
    def _commit_copies(self, cycle_number, inventory, copy_list, comment):
        ...
        for site in sorted(by_site.iterkeys(), key = lambda s: s.name):
            ...
            with signal_blocker:
                ...
                # means we can get rid of self.policy.group below, which means we can let
                # each plugin to decide which group they want to make replicas in
                for copy_id, (approved, site, items) in copy_mapping.iteritems():
                    ...
                    for item in items:
                        ...
                        if type(item) is Dataset:
                            ...
                            if approved:
                                replica = new_replica_from_dataset(item, site, self.policy.group)
